 ...
 */
export async function validateWithdrawalRequest(userId, amount) {
    try {
        ...
        if (!user.username) {
            return {
                valid: false,
                error: "Telegram username is required for withdrawal. Please set your username in Telegram settings.",
            };
        }
        return { valid: true };
    }
 ...
 */
export async function getUserCompletedChannelTasks(userId) {
    try {
      ...
      `)
        ...
        const channelTasks = data
            .filter((ut) => ut.tasks && ut.tasks.type === "channel")
            .map((ut) => ut.tasks);
        return channelTasks;
    }
    catch (error) {
 ...
 */
export async function verifyActiveSubscriptions(userId) {
    try {
 ...
 */
export async function deductStarsForInactiveSubscriptions(userId, inactiveTasks) {
    try {
        let totalDeducted = 0;
...
export async function createWithdrawalRequest(userId, amount) {
    try {
        ...
        const { allActive, inactiveTasks } = await verifyActiveSubscriptions(userId);
        if (!allActive) {
            // Deduct stars for inactive subscriptions
            const deducted = await deductStarsForInactiveSubscriptions(userId, inactiveTasks);
            throw new Error(`Withdrawal rejected: ${inactiveTasks.length} inactive subscription(s) detected. ${deducted} stars deducted. Please re-complete the tasks.`);
        }
 ...
 */
export async function updateWithdrawalStatus(withdrawalId, status, rejectionReason) {
    try {
        const updateData = {
            status,
