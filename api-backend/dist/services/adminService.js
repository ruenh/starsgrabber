...
export async function getAdminStats() {
    try {
        ...
        const totalStarsDistributed = transactionsData?.reduce((sum, t) => sum + Math.abs(t.amount), 0) || 0;
        ...
        const pendingWithdrawalsAmount = withdrawalsData?.reduce((sum, w) => sum + w.amount, 0) || 0;
        return {
...
async function getUserReferralTree(userId) {
    try {
        ...
        const referralNodes = await Promise.all((referrals || []).map((ref) => getUserReferralTree(ref.id)));
        return {
            id: user.id,
...
export async function getReferralTree() {
    try {
        ...
        const trees = await Promise.all((topLevelUsers || []).map((user) => getUserReferralTree(user.id)));
        return trees;
    }
    catch (error) {
