 ...
 */
router.post("/new-task", verifyApiKey, async (req, res) => {
    try {
 ...
 */
router.post("/referral-completion", verifyApiKey, async (req, res) => {
    try {
        ...
        const result = await NotificationController.handleReferralCompletionNotification({
            userId,
            referralName,
            earnings,
            taskTitle,
 ...
 */
router.post("/withdrawal-approved", verifyApiKey, async (req, res) => {
    try {
        ...
        const result = await NotificationController.handleWithdrawalApprovalNotification({
            userId,
            amount,
        });
        res.json(result);
 ...
 */
router.post("/withdrawal-rejected", verifyApiKey, async (req, res) => {
    try {
        ...
        const result = await NotificationController.handleWithdrawalRejectionNotification({
            userId,
            amount,
            reason,
        });
 ...
 */
router.post("/admin-withdrawal-request", verifyApiKey, async (req, res) => {
    try {
        ...
        const result = await NotificationController.handleAdminWithdrawalRequestNotification({
            username,
            amount,
            userId,
            withdrawalId,
        ...
        logger.error("Error in admin-withdrawal-request notification route:", error);
        res.status(500).json({ error: "Internal server error" });
    }
});
