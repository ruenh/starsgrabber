...
export class NotificationController {
    ...
    static async handleNewTaskNotification(data) {
        try {
            await NotificationService.notifyNewTask(data.taskTitle, data.reward, data.taskType);
    ...
    static async handleReferralCompletionNotification(data) {
        try {
            await NotificationService.notifyReferralCompletion(data.userId, data.referralName, data.earnings, data.taskTitle);
    ...
    static async handleWithdrawalApprovalNotification(data) {
        try {
            await NotificationService.notifyWithdrawalApproved(data.userId, data.amount);
            return { success: true, message: "Notification sent" };
        }
        catch (error) {
    ...
    static async handleWithdrawalRejectionNotification(data) {
        try {
            await NotificationService.notifyWithdrawalRejected(data.userId, data.amount, data.reason);
            return { success: true, message: "Notification sent" };
        }
        catch (error) {
    ...
    static async handleAdminWithdrawalRequestNotification(data) {
        try {
            await NotificationService.notifyAdminWithdrawalRequest(data.username, data.amount, data.userId, data.withdrawalId);
            return { success: true, message: "Notification sent" };
        }
        catch (error) {
