...
export class BotActivationService {
     ...
     */
    static async recordActivation(userId, taskId) {
        try {
            ...
            if (error) {
                ...
                if (error.code === "23505") {
                    logger.info(`Bot activation already exists: user ${userId}, task ${taskId}`);
                    return true;
                }
                throw error;
            }
     ...
     */
    static async checkActivation(userId, taskId) {
        try {
     ...
     */
    static generateActivationLink(botUsername, userId, taskId) {
        const startParam = `task_${taskId}_user_${userId}`;
        return `https://t.me/${botUsername}?start=${startParam}`;
    }
}
//# sourceMappingURL=botActivationService.js.map