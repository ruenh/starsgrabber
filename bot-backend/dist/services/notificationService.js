...
export class NotificationService {
    ...
    static async notifyNewTask(taskTitle, reward, taskType) {
        try {
            logger.info(`Sending new task notification: ${taskTitle} (${reward} stars)`);
            ...
            const message = `${taskTypeEmoji} <b>Новое задание!</b>\n\n` +
                `${taskTitle}\n` +
                `Награда: ${reward} ⭐️\n\n` +
            ...
            logger.info(`New task notification complete: ${successCount} sent, ${failCount} failed`);
        }
        catch (error) {
    ...
    static async notifyReferralCompletion(userId, referralName, earnings, taskTitle) {
        try {
            const message = `🎉 <b>Ваш реферал выполнил задание!</b>\n\n` +
                `Пользователь: ${referralName}\n` +
                `Задание: ${taskTitle}\n` +
                `Ваш заработок: ${earnings} ⭐️`;
            await this.notifyUser(userId, message);
            logger.info(`Referral completion notification sent to user ${userId}, earnings: ${earnings}`);
        }
        catch (error) {
            logger.error(`Error sending referral completion notification to user ${userId}:`, error);
        }
    }
    /**
    ...
    static async notifyWithdrawalApproved(userId, amount) {
        try {
            const message = `✅ <b>Ваша заявка на вывод одобрена!</b>\n\n` +
                `Сумма: ${amount} ⭐️\n` +
            ...
            logger.error(`Error sending withdrawal approval notification to user ${userId}:`, error);
        }
    }
    /**
    ...
    static async notifyWithdrawalRejected(userId, amount, reason) {
        try {
            const message = `❌ <b>Ваша заявка на вывод отклонена</b>\n\n` +
                `Сумма: ${amount} ⭐️\n` +
                `Причина: ${reason}`;
            await this.notifyUser(userId, message);
            ...
            logger.error(`Error sending withdrawal rejection notification to user ${userId}:`, error);
        }
    }
    /**
    ...
    static async notifyAdminWithdrawalRequest(username, amount, userId, withdrawalId) {
        try {
            ...
            logger.info(`Admin withdrawal request notification sent for withdrawal #${withdrawalId}`);
        }
        catch (error) {
     ...
     */
    static async batchNotify(userIds, message, delayMs = 50) {
        let successCount = 0;
        let failCount = 0;
